...
export class DataService {
  ...
  static async getJobsForClient(clientId: string): Promise<{ data: Job[] | null; error: any }> {
    try {
  ...
  static async getDocumentsForClient(clientId: string): Promise<{ data: Document[] | null; error: any }> {
    try {
      // First get all jobs for the client
      const { data: jobs, error: jobsError } = await this.getJobsForClient(clientId)
  ...
  static async getMessagesForClient(clientId: string): Promise<{ data: Message[] | null; error: any }> {
    try {
      const { data, error } = await supabase
        .from('messages')
        .select('*')
      ...
      console.error('Error in getMessagesForClient:', error)
      return { data: null, error }
    }
  }

  // Fetch billing information for a specific client
  static async getBillingForClient(clientId: string): Promise<{ data: any[] | null; error: any }> {
    try {
